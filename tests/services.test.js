...
describe('Tests Complets - Services, Repositories et Entités', () => {
  ...
  beforeEach(async () => {
    ...
    const magasin = await db.Magasin.create({ id: 1, nom: 'Magasin Test', adresse: '123 rue test' });
  ...
  describe('Repositories - Opérations CRUD et Logique Métier', () => {
    describe('SequelizeProduitRepository', () => {
        ...
        it('listerTous - Liste tous les produits', async () => {
          ...
          expect(produits.length).toBeGreaterThanOrEqual(1);
        });
      ...
      describe('SequelizeVenteRepository', () => {
        it('sauvegarder - Sauvegarde une vente et ses lignes', async () => {
          const vente = new Vente(null, 1, 1);
          vente.ajouterLigne({ id: 1, prix: 25 }, 2);
          const venteSauvee = await venteRepo.sauvegarder(vente);

          expect(venteSauvee.id).toBeDefined();
          expect(venteSauvee.montantTotal).toBe(50);
          
          const venteAvecLignes = await db.Vente.findByPk(venteSauvee.id, { include: ['lignes'] });
          expect(venteAvecLignes.lignes).toHaveLength(1);
        });
    });

    describe('Associations Sequelize', () => {
      it('Vente inclut Magasin, Utilisateur et Lignes de Vente avec Produit', async () => {
        const venteTrouvee = await db.Vente.findByPk(vente.id, {
            include: [
        ...
        expect(venteTrouvee.lignesDeVente[0].produit.nom).toBe('Laptop');
    });
  });
});
